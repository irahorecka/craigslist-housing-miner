...
class Pickle:
    def pickle_reset(self):
        with open("completed_search.txt", "wb") as pickle_list:
            pickle.dump([], pickle_list)

    def pickle_read(self):
        with open("completed_search.txt", "rb") as pickle_list:
            return pickle.load(pickle_list)

    def pickle_dump(self, item_to_write):
        with open("completed_search.txt", "rb") as pickle_list:
            completed = pickle.load(pickle_list)
        with open("completed_search.txt", "wb") as pickle_list:
            completed.append(item_to_write)
            pickle.dump(completed, pickle_list)
...
def execute_search():
    search_criteria = ExecSearch(sk.state_keys, sk.dist_filters, sk.selected_reg, sk.selected_cat)
